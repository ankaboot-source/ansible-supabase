...
### Role: backup
Deploys pgBackRest as a sibling container sharing the Supabase `db` container's PGDATA volume, with continuous WAL archiving via native pgBackRest async mode, scheduled full + differential backups, point-in-time recovery (`restore.yml`), non-destructive restore verification (`restore-verify.yml`), on-demand backup (`backup.yml`), daily repo-integrity verification, Supabase Storage volume backup, pgsodium root-key backup, and Prometheus metrics + Grafana dashboard + alerting. Repo types: `minio` (local, default), `s3` (external), `posix` (local fs). Encryption is forced ON for external S3 repos. WAL archiving uses pgBackRest's native `archive-async` mode: the pgbackrest binary and config are bind-mounted read-only into the db container, and `archive_command` runs `pgbackrest archive-push %p` directly. pgBackRest manages its own spool, queueing, and retry logic. The pgbackrest sibling container runs the default entrypoint (not a custom archiver script) and handles on-demand backups, stanza management, and repo access. Templates: `pgbackrest.conf.j2`, `docker-compose-backup.yml.j2`, `backup-env.j2`, six `backup-scripts/*.j2`, `grafana/backup.json.j2`, `prometheus-backup-alerts.yml.j2`.
