...
### Role: backup
Deploys pgBackRest as a sibling container sharing the Supabase `db` container's PGDATA volume, with continuous WAL archiving via a shared spool directory, scheduled full + differential backups, point-in-time recovery (`restore.yml`), non-destructive restore verification (`restore-verify.yml`), on-demand backup (`backup.yml`), daily repo-integrity verification, Supabase Storage volume backup, pgsodium root-key backup, and Prometheus metrics + Grafana dashboard + alerting. Repo types: `minio` (local, default), `s3` (external), `posix` (local fs). Encryption is forced ON for external S3 repos. WAL archiving uses a shared spool directory (no docker socket dependency): the db container writes WAL via `archive_command=cp %p <spool>/in/%f`, and the pgbackrest container's `backup-archiver.sh` sidecar processes files from the spool to the repo. Templates: `pgbackrest.conf.j2`, `docker-compose-backup.yml.j2`, `backup-env.j2`, six `backup-scripts/*.j2`, `backup-archiver.sh.j2`, `grafana/backup.json.j2`, `prometheus-backup-alerts.yml.j2`.
