...
Uses `rclone` (via Docker) and the Supabase CLI to dump and upload database backups on a cron schedule.

### Required Variables

```yaml
s3_remote_name: r2          # e.g. r2, gcs, spaces, snowball
s3_provider: Cloudflare     # e.g. GCS, Hetzner, DigitalOcean, Cloudflare
...
```yaml
# Tables to exclude from data dump (comma-separated, schema-qualified)
exclude_tables: "schema.table1,schema.table2"
...
Backups are stored as timestamped `.sql.gz` files (data-only + full schema). Logs go to `/var/log/supabase-backup/`.
